...
// Mock scrape target: a Canvas "Weekly Overview" module page, which
// is typically free-text typed by a teacher rather than structured
// assignment objects. A real scraper would run inside the Chrome
// extension's content script against the rendered module DOM.
const weeklyOverviewPageText = `
Monday: Bring your annotated article for the Socratic seminar.
Tuesday: IXL practice due — check the Math panel for skill codes.
Wednesday: Quiz on Chapters 4-6, bring a pencil, no notes.
Thursday: Field trip permission slips due at the front office.
Friday: Vocabulary quiz, study set posted in the Files tab.
`;
...
/* ================================================================
   4. HIDDEN AGENDA SCRAPER
   Pulls day-labelled, typed-out tasks out of the Weekly Overview
   module body text using a line-anchored regex.
   ================================================================ */
const AGENDA_LINE_REGEX = /^(Monday|Tuesday|Wednesday|Thursday|Friday):\s*(.+)$/gm;

function scrapeWeeklyOverview(pageText) {
  const tasks = [];
  let m;
  while ((m = AGENDA_LINE_REGEX.exec(pageText)) !== null) {
    tasks.push({ day: m[1], task: m[2] });
  }
  return tasks;
}
      ...
      <div class="site-card">
        ...
        <div class="site-actions">
          ${s.linked
            ...
            : `<button data-quicklink="${s.catalogId || s.name}">Link via ClassLink</button>`
          }
        </div>
      </div>`;
  }).join("");

  listEl.querySelectorAll("[data-quicklink]").forEach(btn => {
    btn.addEventListener("click", () => {
      const key = btn.getAttribute("data-quicklink");
      const catalogEntry = homeworkSitesCatalog.find(c => c.id === key);
      const providerUrls = { canvas: "https://canvas.instructure.com", ixl: "https://www.ixl.com", readingplus: "https://student.readingplus.com", deltamath: "https://www.deltamath.com", studyisland: "https://www.studyisland.com", edpuzzle: "https://edpuzzle.com", gclassroom: "https://classroom.google.com", iready: "https://login.i-ready.com" };
      const providerUrl = providerUrls[key];
      if (providerUrl && typeof chrome !== "undefined" && chrome.runtime?.sendMessage) chrome.runtime.sendMessage({ type: "DOCKET_OPEN_TAB", url: providerUrl });
      linkedSites.push({
        catalogId: catalogEntry ? catalogEntry.id : null,
        name: catalogEntry ? catalogEntry.name : key,
        authMethod: "classlink",
        linked: true,
        lastVerified: null,
      });
      renderSiteList();
...
/* ================================================================
   ...
   may hold real data content-script.js scraped from your open
   Canvas tabs. If present, log it and merge in the "Weekly Overview"
   text so the existing scraper/regex pipeline can run on real data
   instead of the mock weeklyOverviewPageText string above.
   Opened as a plain file, chrome.storage is undefined and this is
   a silent no-op — the mock data drives everything, exactly as
   before.
   ================================================================ */
let liveWeeklyOverviewText = null;
let liveAssignments = [];
let liveSubmissions = {};
let liveCourses = [];
let hasCanvasSync = false;
...
function applyLiveSync(scraped) {
  if (!scraped || scraped.providerId !== "canvas") return;
  hasCanvasSync = true;
  liveWeeklyOverviewText = scraped.weeklyOverviewText || null;
  liveAssignments = Array.isArray(scraped.assignments) ? scraped.assignments : [];
  liveSubmissions = scraped.submissions && typeof scraped.submissions === "object" ? scraped.submissions : {};
  liveCourses = Array.isArray(scraped.courses) ? scraped.courses : [];
  ...
  if (syncLine) syncLine.textContent = `Connected to Canvas · ${liveAssignments.length} assignment${liveAssignments.length === 1 ? "" : "s"} · last synced ${stamp}`;
  renderSubjectNav();
  renderPanels();
  renderHiddenAgenda();
}
...
const hiddenAgendaListEl = document.getElementById("hidden-agenda-list");

function currentAssignments() {
  if (!hasCanvasSync) return [];
  const submissions = liveSubmissions || {};
  return [...liveAssignments].sort((a, b) => {
    const aTime = a.dueDate ? Date.parse(`${a.dueDate}T23:59:59`) : Number.POSITIVE_INFINITY;
    const bTime = b.dueDate ? Date.parse(`${b.dueDate}T23:59:59`) : Number.POSITIVE_INFINITY;
...
function submissionFor(assignment) {
  const submissions = liveSubmissions || {};
  return submissions[assignment.id] || submissions[String(assignment.id)] || {};
}
function currentSubmissions() { return hasCanvasSync ? liveSubmissions : {}; }
function currentCourses() { return hasCanvasSync ? liveCourses : []; }
...
function renderAssignmentCard(a) {
  ...
  const overdue = Boolean(a.dueDate) && isOverdue(a.dueDate) && !sub.submitted && !sub.graded && !sub.missing;

  let sideHtml;
  if (sub.missing) {
    sideHtml = `<div class="status-overdue">MISSING</div><button class="launch-btn" data-launch="${a.id}">Open assignment</button>`;
  } else if (sub.graded || sub.workflowState === "graded") {
    sideHtml = `<div class="status-verified">✓ GRADED${sub.grade != null ? ` · ${sub.grade}` : sub.score != null ? ` · ${sub.score}` : ""}</div>`;
  } else if (sub.ixlSmartScore != null && sub.ixlSmartScore >= 80) {
    sideHtml = `
...
function renderPanels() {
  ...
  assignmentPanelsEl.innerHTML = "";

  Object.keys(groups).forEach(subjectKey => {
    if (activeSubjectFilter !== "all" && activeSubjectFilter !== subjectKey) return;
    const meta = SUBJECT_KEYWORDS[subjectKey] || { label: "General", color: "#8a8f9e" };
    const items = groups[subjectKey];

    const block = document.createElement("div");
    block.className = "panel-block";
    block.innerHTML = `
      <div class="panel-block-head">
        <h2><span class="subject-dot" style="--subj-color:${meta.color}"></span> ${meta.label}</h2>
        <span class="count">${items.length} assignment${items.length === 1 ? "" : "s"}</span>
      </div>
      <div>${items.map(renderAssignmentCard).join("")}</div>
    `;
    assignmentPanelsEl.appendChild(block);
  });
...
function renderHiddenAgenda() {
  const tasks = scrapeWeeklyOverview(liveWeeklyOverviewText || weeklyOverviewPageText);
  hiddenAgendaListEl.innerHTML = tasks.map(t => `
    <div class="hidden-agenda-card">
